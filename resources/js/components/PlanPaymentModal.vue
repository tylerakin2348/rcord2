    ...
    <div class="bg-white rounded-lg p-8 w-full max-w-md mx-4">
      ...
      <form @submit.prevent="submitPayment">
        ...
        <div class="mb-4">
          ...
          <input v-model="email" type="email" class="border rounded p-2 w-full" placeholder="Email Address" required />
        </div>
        <div class="mb-4">
          <label class="block text-sm font-medium text-gray-700 mb-2">Card Details</label>
          <div id="stripe-payment-element" class="border rounded p-2 min-h-[48px] min-w-full" style="min-height:48px;width:100%;"></div>
        </div>
...
const mountStripeElements = async () => {
  // Stripe.js is loaded statically, so skip dynamic loader
  if (!window.Stripe) {
  ...
  console.log('stripe.elements type:', typeof stripe.elements)

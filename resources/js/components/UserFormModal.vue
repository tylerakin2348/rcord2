...
const populateForm = () => {
  if (props.user) {
    ...
    form.password = ''
    // Prefer role_id, then role.id, else blank
    if (props.user.role_id) {
      form.role_id = props.user.role_id
      form.role_name = props.user.role_name
    } else if (props.user.roles && props.user.roles[0]?.id) {
      form.role_id = props.user.roles[0]?.id
      form.role_name = props.user.roles[0]?.name
    } else {
      form.role_id = ''
      form.role_name = ''
    }
